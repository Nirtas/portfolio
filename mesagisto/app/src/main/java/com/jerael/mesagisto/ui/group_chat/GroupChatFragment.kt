...
class GroupChatFragment : Fragment() {
    ...
    private lateinit var groupPhotoUrl: String
            ...
            chat_voice_button.setOnTouchListener { v, event ->
                if (checkPermission(RECORD_AUDIO)) {
                    ...
                    if (event.action == MotionEvent.ACTION_DOWN) {
                        ...
                        val messageKey = getMessageKey(groupId)
    ...
    private fun initToolbar() {
        ...
        listenerInfoToolbar = AppValueEventListener {
            val groupData = it.getCommonModel()
            groupName = groupData.fullname
            groupPhotoUrl = groupData.photoUrl
            initInfoToolbar()
        }

        refGroup = REF_DATABASE_ROOT.child(DATABASE_NODE_GROUPS).child(groupId)
        refGroup.addValueEventListener(listenerInfoToolbar)
    ...
    private fun initRecyclerView() {
        ...
        messagesListener = AppChildEventListener {
            ...
            getUserFromDb(message.sender) { data ->
    ...
    override fun onPause() {
        ...
        refGroup.removeEventListener(listenerInfoToolbar)
        refMessages.removeEventListener(messagesListener)
    }

    private fun initInfoToolbar() {
        toolbarInfo.chat_toolbar_fullname_text_view.text = groupName
        toolbarInfo.chat_toolbar_user_photo_circle_image_view.downloadAndSetImage(groupPhotoUrl)
    }
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        APP_ACTIVITY.menuInflater.inflate(R.menu.group_action_menu, menu)
    }

    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.exit_group -> {
                exitGroup(CURRENT_UID, groupId) {
                    showToast(getString(R.string.group_toast_left_group))
                    restartActivity()
                }
                true
            }
